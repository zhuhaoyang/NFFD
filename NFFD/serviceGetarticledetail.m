...
@implementation serviceGetarticledetail
...
- (void)parseResponseData:(NSString *)xml
{
    ...
    if ([items count] == 0 || items == nil) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"获取文章失败!"
                                                       message:nil
                                                      delegate:nil
        ...
        if ([[[items objectAtIndex:0] stringValue] isEqualToString:@"1"]) {
            items = [xmlDoc nodesForXPath:@"//item" error:nil];
...
{
	if (nil != m_delegate
		&& [m_delegate respondsToSelector:@selector(serviceGetarticledetailCallBack:withErrorMessage:)])
	{
		[m_delegate serviceGetarticledetailCallBack:m_arrReceiveData withErrorMessage:error];
	}
}
@end
