...
@implementation serviceDelproduct
...
- (void)parseResponseData:(NSString *)xml
{
    ...
    if ([items count] == 0 || items == nil) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"删除商品失败!"
                                                       message:nil
                                                      delegate:nil
...
{
	if (nil != m_delegate
		&& [m_delegate respondsToSelector:@selector(serviceDelproductCallBack:withErrorMessage:)])
	{
		[m_delegate serviceDelproductCallBack:m_arrReceiveData withErrorMessage:error];
	}
}
