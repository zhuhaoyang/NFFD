...
//    xml = @"<?xml version=\"1.0\" encoding=\"utf-8\"?><efence><nffd><res>xx</res ><types><type><tid>xx</tid></type</types></nffd > </efence>";
    ...
    if ([items count] == 0 || items == nil) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"获取商品类型失败!"
                                                       message:nil
                                                      delegate:nil
        ...
        if ([[[items objectAtIndex:0] stringValue] isEqualToString:@"1"]) {
            items = [xmlDoc nodesForXPath:@"//type" error:nil];
...
{
	if (nil != m_delegate
		&& [m_delegate respondsToSelector:@selector(serviceGettypeCallBack:withErrorMessage:)])
	{
		[m_delegate serviceGettypeCallBack:m_arrReceiveData withErrorMessage:error];
	}
}
