...
@implementation serviceAddproduct
...
- (void)parseResponseData:(NSString *)xml
{
    ...
    if ([items count] == 0 || items == nil) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"添加到购物车失败!"
                                                       message:nil
                                                      delegate:nil
        ...
        if ([[[items objectAtIndex:0] stringValue] isEqualToString:@"1"]) {
            ...
            items = [xmlDoc nodesForXPath:@"//oid" error:nil];
            if ([items count] != 0 && items != nil){
                NSString *strOid = [[items objectAtIndex:0] stringValue];
                [[NSUserDefaults standardUserDefaults] setObject:strOid forKey:@"oid"];
            }
...
{
	if (nil != m_delegate
		&& [m_delegate respondsToSelector:@selector(serviceAddproductCallBack:withErrorMessage:)])
	{
		[m_delegate serviceAddproductCallBack:m_arrReceiveData withErrorMessage:error];
	}
}
