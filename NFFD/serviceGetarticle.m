...
@implementation serviceGetarticle
...
- (void)parseResponseData:(NSString *)xml
{
    ...
    if ([items count] == 0 || items == nil) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"获取文章列表失败!"
                                                       message:nil
                                                      delegate:nil
        ...
        if ([[[items objectAtIndex:0] stringValue] isEqualToString:@"1"]) {
            items = [xmlDoc nodesForXPath:@"//article" error:nil];
...
{
	if (nil != m_delegate
		&& [m_delegate respondsToSelector:@selector(serviceGetarticleCallBack:withErrorMessage:)])
	{
		[m_delegate serviceGetarticleCallBack:m_arrReceiveData withErrorMessage:error];
	}
}
