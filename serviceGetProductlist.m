...
@implementation serviceGetProductlist
...
//    xml = @"<?xml version=\"1.0\" encoding=\"utf-8\"?><efence><nffd><res>xx</res ><types><type><tid>xx</tid></type</types></nffd > </efence>";
    ...
    if ([items count] == 0 || items == nil) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"获取商品列表失败!"
                                                       message:nil
                                                      delegate:nil
...
{
	if (nil != m_delegate
		&& [m_delegate respondsToSelector:@selector(serviceGetProductlistCallBack:withErrorMessage:)])
	{
		[m_delegate serviceGetProductlistCallBack:m_arrReceiveData withErrorMessage:error];
	}
}
